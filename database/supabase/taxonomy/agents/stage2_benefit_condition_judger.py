"""
Stage 2: Benefit-Specific Condition Value Judger
Validates and corrects benefit-condition extractions from the Extractor agent.

Runs in a pillar with BenefitConditionExtractor - processes the same raw_text sequentially.
"""
...
class BenefitConditionJudgerPrompt:
    """Prompt template for benefit-condition extraction judgment."""

    @staticmethod
    def get_system_prompt() -> str:
        """Define agent role and behavior."""
        return """You are a senior insurance policy analyst responsible for quality assurance of extracted benefit-specific condition data.

Your task is to validate benefit-condition extractions LIST and either approve them or provide corrections for EACH ITEM.

**VALIDATION CRITERIA:**
1. **Correctness:** Extracted data accurately reflects the policy text
2. **Relevance:** Information is relevant to BOTH the benefit AND the condition
3. **Format:** JSON structure exactly matches requirements
4. **Completeness:** All extractable parameters are included
5. **Accuracy:** original_text is verbatim from source (no paraphrasing)
6. **Precision:** Parameters use correct data types (numbers as numeric, not strings)
7. **Specificity:** Condition is benefit-specific, not a general policy condition
  ...
  "validations": [
    {
      ...
      "condition": "exact_condition_name",
      "approve": true,
      "final_value": {
        "benefit_name": "exact_benefit_name",
        "condition": "exact_condition_name",
...
**Reference (Benefit, Condition) Pairs ({total} total):**
[{pairs_str}...]

**VALIDATION CHECKLIST (for EACH item):**
1. Does the extraction accurately represent information in the policy text?
2. Is the (benefit_name, condition) pair EXACTLY from the reference list?
3. Is the product_name EXACTLY "{product_name}"?
4. Is the original_text verbatim (not paraphrased)?
5. Are all relevant parameters extracted?
6. Are numeric values stored as numbers (not strings)?
7. Does the JSON structure match requirements?
8. Is condition_exist correctly set?
9. Is this a BENEFIT-SPECIFIC condition (not a general policy condition)?
...
{{
  "validations": [
    {{
      ...
      "condition": "condition_from_extraction",
      "approve": true,
...
class BenefitConditionJudger:
    """Judges list of benefit-condition extractions."""
    ...
    def __init__(
        ...
        """
        Initialize benefit-condition judger.

        Args:
            api_client: API client for LLM calls
            benefit_condition_pairs: List of valid (benefit_name, condition) tuples (reference)
        """
        self.api_client = api_client
        self.benefit_condition_pairs = benefit_condition_pairs
        self.prompt = BenefitConditionJudgerPrompt()
    ...
    def judge_extractions(
        ...
        """
        Judge a list of benefit-condition extraction results.

        Args:
            extraction_result: Result from BenefitConditionExtractor (contains list in extracted_data)
...
class BatchBenefitConditionJudger:
    """Parallel batch processor for benefit-condition judgment."""

    def __init__(
        self,
        judger: BenefitConditionJudger,
        output_dir: Path
    ):
        """
        ...
        Args:
            judger: BenefitConditionJudger instance
    ...
    def judge_extractions(
        ...
        """
        print(f"\n{'=' * 80}")
        print(f"Judging All Benefit-Condition Extractions")
        print(f"{'=' * 80}")
