"""
Stage 2: General Condition Value Judger
Validates and corrects condition extractions from the Extractor agent.

Runs in a pillar with ConditionExtractor - processes the same raw_text sequentially.
"""
...
class ConditionJudgerPrompt:
    """Prompt template for condition extraction judgment."""

    @staticmethod
    def get_system_prompt() -> str:
        """Define agent role and behavior."""
        return """You are a senior insurance policy analyst responsible for quality assurance of extracted policy data.

Your task is to validate condition extractions LIST and either approve them or provide corrections for EACH ITEM.

**VALIDATION CRITERIA:**
1. **Correctness:** Extracted data accurately reflects the policy text
2. **Relevance:** Information is relevant to the specified condition
...
If approve=false, final_value MUST contain your corrected version."""
    ...
    def get_user_prompt(
        ...
        conditions_str = ', '.join(f'"{c}"' for c in condition_names[:20])
...
8. Is condition_exist correctly set (true if mentioned, false if not)?
...
{{
  "validations": [
    {{
      "item_index": 0,
      "condition": "condition_name_from_extraction",
      "approve": true,
...
class ConditionJudger:
    """Judges list of condition extractions."""
    ...
    def __init__(
        ...
        """
        Initialize condition judger.

        Args:
            api_client: API client for LLM calls
            condition_names: List of valid condition names (reference)
        """
        self.api_client = api_client
        self.condition_names = condition_names
        self.prompt = ConditionJudgerPrompt()
    ...
    def judge_extractions(
        ...
        """
        Judge a list of condition extraction results.

        Args:
            extraction_result: Result from ConditionExtractor (contains list in extracted_data)
...
class BatchConditionJudger:
    """Parallel batch processor for condition judgment."""

    def __init__(
        self,
        judger: ConditionJudger,
        output_dir: Path
    ):
        """
        ...
        Args:
            judger: ConditionJudger instance
    ...
    def judge_extractions(
        ...
        """
        print(f"\n{'=' * 80}")
        print(f"Judging All Condition Extractions")
        print(f"{'=' * 80}")
